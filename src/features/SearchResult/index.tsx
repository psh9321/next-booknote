...
export const SearchResult = ({ type } : SEARCH_RESULT) => {
    ...
    const { data, isFetching, fetchNextPage, hasNextPage } = useSearchResultQueryHook(type, searchKeyword);
    ...
    return (
        <article className="relative py-[40px] bg-[#2A2F32] space-y-[20px]">
        { 
            data &&
            isEmpty ? 
            ...
            data?.pages.map(page => {
                ...
                return <BookList className="mt-[0px] py-[0px]" key={JSON.stringify(page)} data={item} />
            })
