"use client";

import Image from "next/image";
import Link from "next/link";

import { useRef } from "react";

import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation } from 'swiper/modules';
import type { NavigationOptions } from 'swiper/types';

import { ChevronLeft, ChevronRight } from 'lucide-react';

import { useBestSellerListHook } from "../../hook/useQuery";

export const BestSellerList = () => {
    const { data, isSuccess, isError } = useBestSellerListHook();

    const prevRef = useRef<HTMLButtonElement>(null);
    const nextRef = useRef<HTMLButtonElement>(null);

    const SetNavBtnsCallback = (swiper: { isBeginning: boolean; isEnd: boolean }) => {
        if (prevRef.current) prevRef.current.disabled = swiper.isBeginning;
        if (nextRef.current) nextRef.current.disabled = swiper.isEnd;
    };

    if(isError || !isSuccess) return <></>;

    return (
        <div className="flex items-center gap-4 mt-[20px] py-[40px] px-[20px] bg-[#2A2F32] rounded-[10px]
        
            [&>button]:shrink-0
            [&>button]:w-8
            [&>button]:h-8
            [&>button]:flex
            [&>button]:items-center
            [&>button]:justify-center
            [&>button]:rounded-full
            [&>button]:bg-gray-200
            [&>button]:hover:bg-gray-300
            [&>button:disabled]:opacity-30
            [&>button:disabled]:cursor-not-allowed
            [&>button>svg]:stroke-[#000]
        ">

            <button ref={prevRef}><ChevronLeft/></button>
            <button ref={nextRef} className="order-last"><ChevronRight/></button>

            <Swiper
                modules={[Navigation]}
                wrapperTag="ol"
                slidesPerView={5}
                spaceBetween={30}
                navigation
                onBeforeInit={(swiper) => {
                    (swiper.params.navigation as NavigationOptions).prevEl = prevRef.current;
                    (swiper.params.navigation as NavigationOptions).nextEl = nextRef.current;
                }}
                onInit={SetNavBtnsCallback}
                onSlideChange={SetNavBtnsCallback}
            >
                {
                    (data as ALADIN.BOOK_ITEM[]).map((el, i) => (
                        <SwiperSlide tag="li" key={`${JSON.stringify(el)}-${i}`}>
                            <Link href={`/book/${el["isbn"]}`} className="relative block w-[150px] h-[200px]">
                                <Image
                                    src={el["cover"]}
                                    alt={`${el["title"]} 커버 이미지`}
                                    fill
                                    sizes="auto"
                                    loading="eager"
                                    className="rounded-[10px]"
                                />
                            </Link>
                        </SwiperSlide>
                    ))
                }
                
            </Swiper>
        </div>
    );
};
