...
import { useMyBookNoteQueryHook } from "@/hook/useQuery"

const MyMyBookNotePageView = () => {

    const session = useSession();

    const { data } = useMyBookNoteQueryHook(session.data?.user.id);
            ...
            <section className="w-[calc(100%-80px)] mx-auto">
                ...
                <ul className="flex flex-wrap w-full">
                {
                    data?.pages.map(page => {
                        ...
                        const list = (page as MY_BOOK_NOTE_RESPONSE)["list"];

                        return list.map((el, i) => {

                            // `${process.env.NEXT_PUBLIC_FILE_DIRECTORY}/${userId}/${el["contentsImages"][0]}`
                            return (
                                <li key={`내가등록한도서-${el["_id"]}-${i}`} className="group w-[25%] border-1 border-[#0c1014]">
                                    <Link href={""} className="relative block w-full h-[300px]">
                                        ...
                                        <Image sizes="auto" loading="eager" src={`${process.env.NEXT_PUBLIC_FILE_DIRECTORY}/${el["_id"]}/${el["contentsImages"][0]}`} alt={`${el["_id"]}-이미지`} fill className="object-cover" unoptimized />
...
export default MyMyBookNotePageView
